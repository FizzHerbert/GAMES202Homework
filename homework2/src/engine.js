...
async function GAMES202Main() {
	...
	for (let i = 0; i < envmap.length; i++) {
		...
		let val = '';
		let mary_shadowed_100 = '/transport_mary_Shadowed_100.txt';
		let mary_unshadowed_100 = '/transport_mary_UnShadowed_100.txt';
		let mary_interreflection_bounce_1_100 = '/transport_mary_Interreflection_bounce_1_100.txt';
		await this.loadShaderFile(envmap[i] + mary_interreflection_bounce_1_100).then(result => {
			val = result;
		});
		// await this.loadShaderFile(envmap[i] + mary_shadowed_100).then(result => {
		// 	val = result;
		// });
		// await this.loadShaderFile(envmap[i] + mary_unshadowed_100).then(result => {
		// 	val = result;
		// });
	...
	loadOBJ(renderer, 'assets/mary/', 'mary', 'CubeMapMaterial', box2Transform);
