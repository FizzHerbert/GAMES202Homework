...
namespace ProjEnv
{
    ...
    {
        return std::atan2(x * y, std::sqrt(x * x + y * y + 1.0)); //防止出现0乘0的情况
    }
                                                    ...
                                                    const int &channel)
    {
        ...
        constexpr int SHNum = (SHOrder + 1) * (SHOrder + 1);//SHOrder为使用的球谐函数, 计算总共使用的球谐函数个数
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);//for RGB
                    float solid_angle_ = CalcArea(x, y, width, height);
                    for (int l = 0; l <= SHOrder; l++)
                    {
                        for (int m = -l; m <= l; m++)//注意l不要用size_t类型，否则会导致只计算第一个球谐函数的系数
                        {
                            //要求dir是unit的,另外EvalSH函数要求的dir为vector3d类型，eigen需要显示转换
                            SHCoeffiecents[sh::GetIndex(l, m)] += Le * sh::EvalSH(l, m, dir.cast<double>().normalized()) * solid_angle_;
                        }
                    }              
                }
            }
...
public:
    ...
    std::vector<std::string> file_path_suffix = { "Unshadowed", "Shadowed", "Interreflection" };
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        // Here only compute one mesh and get the name of obj
        const auto mesh = scene->getMeshes()[0];
        std::string obj_name = mesh->getName();
        size_t obj_name_start = obj_name.find_last_of("\\") + 1;
        obj_name = obj_name.substr(obj_name_start, obj_name.size() - obj_name_start - 4);
        std::string bounce_name = "";
        if (m_Type == Type::Interreflection)
        {
            bounce_name = "bounce_";
            bounce_name += std::to_string(m_Bounce);
        }
        // Projection environment
        auto cubePath = getFileResolver()->resolve(m_CubemapPath);
        auto lightPath = cubePath / "light.txt";//环境光因为没有采样和阴影的问题，不需要生成多个
        auto transPath = cubePath / (std::string("transport_") + obj_name +  "_" + file_path_suffix[(int)m_Type] + "_" 
                            + bounce_name + "_" + std::to_string(m_SampleCount) + std::string(".txt"));
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                if (m_Type == Type::Unshadowed)
                {
                    ...
                    //对于所有的pre-calculated 的球面均匀分布的随机方向射线
                    //计算当前射线sample 的cosine
                    double H = wi.dot(n);//Dot(射线采样的vec3 ，法线)
                    //如果射线在上半球...
                    if (H > 0.0)
                    {
                        //那么就投影到SH space
                        return H;//其余的计算在函数sh::ProjectFunction中进行
                    }
                    //这时BRDF=0
                    return 0;
                }
                else
                {
                    ...
                    {
                        //如果光线到达该顶点之前没有被阻挡
                        Ray3f ray = Ray3f(v, wi);//从顶点v发出的方向为wi的光线
                        if (!scene->rayIntersect(ray))//判断是否与场景相交
                        {
                            //若没有相交，那么visibility=1,就投影到SH space
                            return H;//其余的计算在函数sh::ProjectFunction中进行
                        }
                    }

                    //否则要么visibility=0,要么BRDF=0
                    return 0;
                }
            };
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            Eigen::MatrixXf old_TransportSHCoeffs = m_TransportSHCoeffs;//上一个bounce的间接光照的球谐系数
            
            for (size_t i = 0; i < m_Bounce; i++)//计算迭代bounce次
            {
                Eigen::MatrixXf new_TransportSHCoeffs = Eigen::MatrixXf::Zero(SHCoeffLength, mesh->getVertexCount());////下一个bounce的间接光照的球谐系数

                for (size_t j = 0; j < mesh->getVertexCount(); j++)//对每个顶点
                {
                    const Point3f& v = mesh->getVertexPositions().col(j);
                    const Normal3f& n = mesh->getVertexNormals().col(j).normalized();
                    ...
                    for (int t = 0; t < sample_side; t++) 
                    {
                        for (int p = 0; p < sample_side; p++) 
                        {
                            ...
                            {                     
                                ...
                                {
                                    //注意要考虑发出光线与面片交界处点的法线方向
                                    const Vector3f& bary = its.bary;
                                    const Normal3f& n0 = mesh->getVertexNormals().col(its.tri_index.x());
                                    const Normal3f& n1 = mesh->getVertexNormals().col(its.tri_index.y());
                                    const Normal3f& n2 = mesh->getVertexNormals().col(its.tri_index.z());
                                    const Normal3f& n_intersection = bary.x() * n0 + bary.y() * n1 + bary.z() * n2;
                                    double H_intersection = -wi.dot(n_intersection);
                                    if (H_intersection > 0.0)
                                    {
                                        const Eigen::Matrix<Vector3f::Scalar, SHCoeffLength, 1> sh0 = old_TransportSHCoeffs.col(its.tri_index.x());
                                        const Eigen::Matrix<Vector3f::Scalar, SHCoeffLength, 1> sh1 = old_TransportSHCoeffs.col(its.tri_index.y());
                                        const Eigen::Matrix<Vector3f::Scalar, SHCoeffLength, 1> sh2 = old_TransportSHCoeffs.col(its.tri_index.z());
                                        const Eigen::Matrix<Vector3f::Scalar, SHCoeffLength, 1> sh_intersection = bary.x() * sh0 + bary.y() * sh1 + bary.z() * sh2;
                                        //double distance2 = (its.p - v).dot(its.p - v) * 32 * 32;
                                        //需要乘上两个cos
                                        new_TransportSHCoeffs.col(j) = new_TransportSHCoeffs.col(j) + sh_intersection * H * H_intersection;
                                        //intersect_count++;
                                    }
                                }
                            }
                    ...
                    new_TransportSHCoeffs.col(j) *= weight;
                    //std::cout << intersect_count << " " << new_TransportSHCoeffs.col(j)[0] << std::endl;
                    m_TransportSHCoeffs.col(j) += new_TransportSHCoeffs;
                }
                old_TransportSHCoeffs = new_TransportSHCoeffs;
            }
        }
    ...
    Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &ray) const
    {
        ...
        //tri_index获取与光线相交的三角面片各个点在所有点中的序号
        //x()/y()/z()分别得到三角面片三个点各自的序号
