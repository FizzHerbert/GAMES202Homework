...
int resolution = 128;
const int resolution_const = 128;
int channel = 3;
...
Vec3f ImportanceSampleGGX(Vec2f Xi, Vec3f N, float roughness) {
    ...
    //球面坐标系
    float sinTheta = a * sqrt(Xi.x);//thetam
    float cosTheta = sqrt(1.f - Xi.x);
    float phi = 2 * PI * Xi.y;//phim

    //TODO: from spherical space to cartesian space - Bonus 1
    //球坐标系转化为笛卡尔坐标系
    float carX = sinTheta * cos(phi);
    float carY = sinTheta * sin(phi);
    float carZ = cosTheta;
    Vec3f wi = Vec3f(carX, carY, carZ);

    //TODO: tangent coordinates - Bonus 1
    //以N为轴构建坐标系
    float sq = sqrtf(N.y * N.y + N.z * N.z);
    Vec3f X = Vec3f(sq, -N.x * N.y / sq, -N.x * N.z / sq);//为单位向量，且与N正交
    Vec3f Y = cross(N, X);
    Vec3f Z = N;

    //TODO: transform H to tangent space - Bonus 1
    //笛卡尔坐标系转为切线坐标系
    Vec3f H = Vec3f(dot(wi, X), dot(wi, Y), dot(wi, Z));
    H = normalize(H);
...
int main() {
	...
	else 
    {
        ...
        uint8_t data[resolution_const * resolution_const * 3];
		...
		stbi_write_png("GGX_Eavg_LUT_IS.png", resolution, resolution, channel, data, 0);
	}
